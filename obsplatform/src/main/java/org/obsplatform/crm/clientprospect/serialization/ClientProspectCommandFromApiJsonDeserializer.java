package org.obsplatform.crm.clientprospect.serialization;
...
public class ClientProspectCommandFromApiJsonDeserializer {
	...
	private final Set<String> supportedParameters = new HashSet<String>(Arrays.asList("id", "sourceOfPublicityInt", "preferredPlanInt",
					"city", "sourceOther", "prospectType", "firstName", "middleName", "lastName", "homePhoneNumber",
					"workPhoneNumber", "mobileNumber", "email", "sourceOfPublicity", "preferredCallingTime", "note",
					"address", "streetArea", "cityDistrict", "state", "country", "locale", "preferredPlan", "status",
					"statusRemark", "callStatus", "assignedTo", "notes", "isDeleted", "zipCode"));
	...
	public void validateForCreate(final String json) {
		...
		/*
		 * final String email = fromApiJsonHelper.extractStringNamed("email",
		 * element);
		 ...
		 * fromApiJsonHelper.extractStringNamed("streetArea", element);
		 * baseDataValidator
		 * .reset().parameter("streetArea").value(streetArea).notBlank
		 * ().notExceedingLengthOf(100);
		 */
		 ...
		 * fromApiJsonHelper.extractStringNamed("zipCode", element);
		 * baseDataValidator
		 * .reset().parameter("zipCode").value(zipCode).notBlank
