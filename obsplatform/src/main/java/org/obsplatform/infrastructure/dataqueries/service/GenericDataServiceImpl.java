...
import org.obsplatform.infrastructure.dataqueries.data.ResultsetColumnValueData;
...
public class GenericDataServiceImpl implements GenericDataService {
	...
	@Override
	public String replace(final String str, final String pattern,
	...
	@Override
	public String generateJsonFromGenericResultsetData(
		...
		final List<ResultsetColumnHeaderData> columnHeaders = grs
				.getColumnHeaders();
		...
		for (int i = 0; i < data.size(); i++) {
			...
			for (int j = 0; j < rSize; j++) {
				...
				writer.append(doubleQuote
						+ columnHeaders.get(j).getColumnName() + doubleQuote
						+ ": ");
				...
				if (currVal != null && currColType != null) {
					if (currColType.equals("DECIMAL")
						...
						if (currColType.equals("DATE")) {
							...
							writer.append("[" + localDate.getYear() + ", "
									+ localDate.getMonthOfYear() + ", "
									+ localDate.getDayOfMonth() + "]");
						} else {

							writer.append(doubleQuote
									+ replace(currVal, doubleQuote,
											slashDoubleQuote) + doubleQuote);
						}
					}
				} else {
					writer.append("null");
	...
	public List<ResultsetColumnHeaderData> fillResultsetColumnHeaders(
		...
		columnDefinitions.beforeFirst();// Moves the cursor to the front of this
										// RowSet, just before the first row
		while (columnDefinitions.next()) {
			final String columnName = columnDefinitions
					.getString("COLUMN_NAME");
			final String isNullable = columnDefinitions
					.getString("IS_NULLABLE");
			final String isPrimaryKey = columnDefinitions
					.getString("COLUMN_KEY");
			final String columnType = columnDefinitions.getString("DATA_TYPE");
			final Long columnLength = columnDefinitions
					.getLong("CHARACTER_MAXIMUM_LENGTH");

			final boolean columnNullable = "YES".equalsIgnoreCase(isNullable);
			final boolean columnIsPrimaryKey = "PRI"
			...
			final ResultsetColumnHeaderData rsch = ResultsetColumnHeaderData
					.detailed(columnName, columnType, columnLength,
							columnNullable, columnIsPrimaryKey, columnValues,
							codeName);
	...
	private List<ResultsetColumnValueData> retreiveColumnValues(
		...
		final String sql = "select v.id, v.code_id, v.code_value from m_code m "
		...
		while (rsValues.next()) {
			...
			final Integer codeId = rsValues.getInt("code_id");

			columnValues
					.add(new ResultsetColumnValueData(id, codeValue, codeId));
		}
	...
	private List<ResultsetColumnValueData> retreiveColumnValues(
		...
		if (codeId != null) {
			final String sql = "select v.id, v.code_value from m_code_value v where v.code_id ="
					+ codeId + " order by v.order_position, v.id";
			final SqlRowSet rsValues = this.jdbcTemplate.queryForRowSet(sql);
			rsValues.beforeFirst();
			while (rsValues.next()) {
				final Integer id = rsValues.getInt("id");
	...
	private SqlRowSet getDatatableMetaData(final String datatable) {
		...
		final SqlRowSet columnDefinitions = this.jdbcTemplate
				.queryForRowSet(sql);
	...
	private SqlRowSet getDatatableCodeData(final String datatable,
