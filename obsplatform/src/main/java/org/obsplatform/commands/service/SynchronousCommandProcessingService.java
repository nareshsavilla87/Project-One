...
import org.obsplatform.commands.exception.RollbackTransactionAsCommandIsNotApprovedByCheckerException;
import org.obsplatform.commands.exception.UnsupportedCommandException;
...
public class SynchronousCommandProcessingService implements CommandProcessingService {
	...
	private NewCommandSourceHandler findCommandHandler(final CommandWrapper wrapper) {
		...
		if (wrapper.isConfigurationResource()) {
	      ...
	      }else if(wrapper.isChargeVariant()){
			  if(wrapper.isCreate()){
				 handler = applicationContext.getBean("createChargeVariantCommandHandler",NewCommandSourceHandler.class);
			  }else if(wrapper.isUpdate()){
					 handler = applicationContext.getBean("updateChargeVariantCommandHandler",NewCommandSourceHandler.class);
			  }else if(wrapper.isDelete()){
					 handler = applicationContext.getBean("deleteChargeVariantCommandHandler",NewCommandSourceHandler.class);
			  }else {
				 throw new UnsupportedCommandException(wrapper.commandName());
			  }
	      }
