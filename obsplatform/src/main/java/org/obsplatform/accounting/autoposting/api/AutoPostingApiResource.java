 ...
 */
package org.obsplatform.accounting.autoposting.api;
...
import org.obsplatform.accounting.closure.data.GLClosureData;
import org.obsplatform.accounting.closure.service.GLClosureReadPlatformService;
...
@Path("/autopostingrules")
@Component
@Scope("singleton")
public class AutoPostingApiResource {
    ...
    @Autowired
    public AutoPostingApiResource(final PlatformSecurityContext context, final GLClosureReadPlatformService glClosureReadPlatformService,
    ...
    public String retreiveClosure(@PathParam("glClosureId") final Long glClosureId, @Context final UriInfo uriInfo) {
        ...
        if (settings.isTemplate()) {
            // glClosureData.setAllowedOffices(new
            // ArrayList<OfficeLookup>(this.officeReadPlatformService.retrieveAllOfficesForLookup()));
        }
