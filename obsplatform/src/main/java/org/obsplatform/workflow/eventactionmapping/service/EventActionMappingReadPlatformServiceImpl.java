...
public class EventActionMappingReadPlatformServiceImpl implements EventActionMappingReadPlatformService {
	...
	@Autowired
	public EventActionMappingReadPlatformServiceImpl(
	...
	public List<EventActionMappingData> retrieveAllEventMapping() {
		...
		try {
			...
			final EventActionMapper mapper = new EventActionMapper();
	...
	private static final class EventActionMapper implements
			RowMapper<EventActionMappingData> {
		...
		public EventActionMappingData mapRow(final ResultSet rs,
			...
			return new EventActionMappingData(id, eventName, actionName,
					process, isDeleted);
		}
	}
	...
	public List<McodeData> retrieveEventMapData(String str) {
		...
		final String sql = "select " + mapper.schema() + " m.code_name=?";
	...
	private static final class ItemDataMaper implements RowMapper<McodeData> {
		...
		@Override
		public McodeData mapRow(final ResultSet rs, final int rowNum)
				throws SQLException {
	...
	public EventActionMappingData retrieveEventActionDetail(Long discountId) {
		try {
			context.authenticatedUser();
			final EventActionMapper mapper = new EventActionMapper();
			final String sql = "select " + mapper.schemaForSingle() + " where em.id=?";
	...
	public List<EventActionMappingData> retrieveEvents(String event) {
		...
		final String sql = "select " + mapper.schema() + "  where b.event_name=? and b.is_deleted='N'";
		
		return this.jdbcTemplate.query(sql, mapper, new Object[] { event });
		
	}

	private static final class EventMapper implements
			RowMapper<EventActionMappingData> {
		...
		public EventActionMappingData mapRow(final ResultSet rs,
			...
			// LocalDate startDate = JdbcSupport.getLocalDate(rs,"startDate");
			// String status = rs.getString("status");
			return new EventActionMappingData(null, null, actionName, null,
					null, null, null);
