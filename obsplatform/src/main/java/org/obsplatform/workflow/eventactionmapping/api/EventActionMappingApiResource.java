...
public class EventActionMappingApiResource {
	...
	private final Set<String> RESPONSE_DATA_PARAMETERS = new HashSet<String>(
			Arrays.asList("id", "action", "event", "process"));
	...
	@Autowired
	public EventActionMappingApiResource(
	...
	public String retrieveEventActionMapTemplate(@Context final UriInfo uriInfo) {
		...
		context.authenticatedUser().validateHasReadPermission(resourceNameForPermissions);
		final List<McodeData> templateData = this.eventActionMappingReadPlatformService.retrieveEventMapData("Action");
		final List<McodeData> templateData1 = this.eventActionMappingReadPlatformService.retrieveEventMapData("Events");
		final EventActionMappingData data = new EventActionMappingData(templateData,templateData1);
		final ApiRequestJsonSerializationSettings settings = apiRequestParameterHelper.process(uriInfo.getQueryParameters());
	...
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String retrieveSingleDiscountDetails(@PathParam("id") final Long id,
			@Context final UriInfo uriInfo) {

		context.authenticatedUser().validateHasReadPermission(resourceNameForPermissions);
		final EventActionMappingData discountMasterData = this.eventActionMappingReadPlatformService.retrieveEventActionDetail(id);
	...
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String updateEventAction(@PathParam("id") final Long id,
