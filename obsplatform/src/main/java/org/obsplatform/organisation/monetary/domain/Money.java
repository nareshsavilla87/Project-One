 ...
 */
package org.obsplatform.organisation.monetary.domain;
...
public class Money implements Comparable<Money> {
	...
	public static Money of(final MonetaryCurrency currency,
			final BigDecimal newAmount) {
		return new Money(currency.getCode(), currency.getDigitsAfterDecimal(),
	...
	private Money(final String currencyCode, final int digitsAfterDecimal,
	...
	public Money copy() {
		return new Money(this.currencyCode, this.currencyDigitsAfterDecimal,
	...
	public Money minus(final BigDecimal amountToSubtract) {
		if (amountToSubtract == null
				|| amountToSubtract.compareTo(BigDecimal.ZERO) == 0) {
			return this;
		}
	...
	public Money dividedBy(final BigDecimal valueToDivideBy,
			final RoundingMode roundingMode) {
		if (valueToDivideBy.compareTo(BigDecimal.ONE) == 0) {
			return this;
		}
		BigDecimal newAmount = this.amount
				.divide(valueToDivideBy, roundingMode);
		return Money.of(monetaryCurrency(), newAmount);
	}

	public Money dividedBy(final double valueToDivideBy,
			final RoundingMode roundingMode) {
		if (valueToDivideBy == 1) {
			return this;
		}
		BigDecimal newAmount = this.amount.divide(
				BigDecimal.valueOf(valueToDivideBy), roundingMode);
		return Money.of(monetaryCurrency(), newAmount);
	}
	...
	public Money multipliedBy(final double valueToMultiplyBy) {
		...
		BigDecimal newAmount = this.amount.multiply(BigDecimal
				.valueOf(valueToMultiplyBy));
		return Money.of(monetaryCurrency(), newAmount);
	}
	...
	public Money multiplyRetainScale(final BigDecimal valueToMultiplyBy,
			final RoundingMode roundingMode) {
		if (valueToMultiplyBy.compareTo(BigDecimal.ONE) == 0) {
			return this;
		}
		BigDecimal newAmount = this.amount.multiply(valueToMultiplyBy);
		newAmount = newAmount.setScale(this.currencyDigitsAfterDecimal,
				roundingMode);
		return Money.of(monetaryCurrency(), newAmount);
	}

	public Money multiplyRetainScale(final double valueToMultiplyBy,
			final RoundingMode roundingMode) {
		return this.multiplyRetainScale(BigDecimal.valueOf(valueToMultiplyBy),
				roundingMode);
	}
	...
	public String toString() {
		return new StringBuilder().append(this.currencyCode).append(' ')
				.append(this.amount.toPlainString()).toString();
	}
	...
	private MonetaryCurrency monetaryCurrency() {
		return new MonetaryCurrency(this.currencyCode,
