...
public class MessageGmailBackedPlatformEmailService implements MessagePlatformEmailService {
	...
	public String sendToUserMobile(String message, Long id) {
		...
		try {
			...
			//urlconnection.setRequestProperty("ContentType", "application/xwwwformurlencoded");
			...
			String output = retval.substring(0, 3);
			...
			String splitParams[] = message.split("\\?");
			
			String url = splitParams[0]+"?";
			String queryParam = splitParams[1];
			
			url = url + URLEncoder.encode(queryParam);
