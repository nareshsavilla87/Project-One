	...
	@Override
		public List<PriceData> retrieveAllPrices(Long planId,String billingFreq, Long clientId ) {
			...
			String sql ="SELECT da.id AS id,if(da.service_code = 'None', 0, se.id) AS serviceId,da.service_code AS service_code,da.charge_code AS charge_code," +
				     ...
				     " WHERE da.is_deleted = 'n' AND ca.address_key = 'PRIMARY' AND da.plan_id = ?  AND (c.billfrequency_code= ? OR c.billfrequency_code IS NULL) " +
				     ...
				     " AND d.state = c.state_name AND d.address_key = 'PRIMARY' AND d.client_id = ? and a.plan_id=?  AND (cc.billfrequency_code= ? OR cc.billfrequency_code IS NULL)),0)" +
				     ...
				     " AND c.country_name = d.country AND d.address_key = 'PRIMARY' AND d.client_id = ? AND a.plan_id = ? AND (cc.billfrequency_code= ? OR cc.billfrequency_code IS NULL)),0))" +
				     " GROUP BY da.id";
