		  ...
		  String sql = null;
            if(clientId != null){
            	sql =" SELECT s.id AS id, s.plan_code AS planCode, s.is_prepaid AS isPrepaid" +
            		 ...
            		 "  WHERE s.plan_status = 1 AND s.is_deleted = 'n' AND s.id != 0 and p.price_region_id = pd.priceregion_id" +
            	...
            	sql = "select s.id as id,s.plan_code as planCode,s.is_prepaid as isPrepaid from b_plan_master s " +
