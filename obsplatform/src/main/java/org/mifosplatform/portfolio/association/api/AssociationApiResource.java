...
public class AssociationApiResource {
	...
	private final Set<String> RESPONSE_DATA_PARAMETERS = new HashSet<String>(
			Arrays.asList("id", "planCode", "OrderId", "hardware", "serialNo",
	...
	public String retrieveAssociationDetails(
			@PathParam("clientId") final Long clientId,
			@Context final UriInfo uriInfo) {

		context.authenticatedUser().validateHasReadPermission(
				resourceNameForPermissions);
		List<AssociationData> associationDatas = this.associationReadplatformService
				.retrieveClientAssociationDetails(clientId, null);
		final ApiRequestJsonSerializationSettings settings = apiRequestParameterHelper
				.process(uriInfo.getQueryParameters());
		return this.toApiJsonSerializer.serialize(settings, associationDatas,
	...
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String retrieveAssociation(
			@PathParam("clientId") final Long clientId,
		...
		try {
			...
			context.authenticatedUser().validateHasReadPermission(
					resourceNameForPermissions);
			AssociationData associationData = this.associationReadplatformService
					.retrieveSingleDetails(id);
			Collection<EnumValuesData> enumValuesDatas = this.enumReadplaformService
					.getEnumValues(EnumValuesConstants.ENUMVALUE_PROPERTY_DEVICE_SWAP);
			List<AssociationData> HardwareDatas = this.associationReadplatformService
					.retrieveHardwareData(clientId);
			List<AssociationData> planDatas = this.associationReadplatformService
					.retrieveplanData(clientId);
			HardwareDatas.add(new AssociationData(associationData
					.getSerialNum(), associationData.getProvisionNumber(),
					associationData.getAllocationType(), associationData
							.getPropertyCode(), associationData.getOrderId()));

			AssociationData data = new AssociationData(
					associationData.getPlanId(), associationData.getPlanCode(),
					associationData.getOrderId());
			planDatas.add(data);
			...
			return this.toApiJsonSerializer.serialize(settings,
					associationData, RESPONSE_DATA_PARAMETERS);
	...
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String retrieveAssociationTemplate(
	...
	public String addAssociation(@PathParam("clientId") final Long clientId,
		...
		final CommandWrapper commandRequest = new CommandWrapperBuilder()
				.createAssociation(clientId).withJson(apiRequestBodyAsJson)
				.build();
	...
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String deAssoicationHardware(
			@PathParam("associationId") final Long associationId,
			final String apiRequestBodyAsJson) {
		final CommandWrapper commandRequest = new CommandWrapperBuilder()
				.updateDeAssociation(associationId)
	...
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String retrieveAssociationWithSerialNum(
			@PathParam("clientId") final Long clientId,
		...
		try {
			...
			context.authenticatedUser().validateHasReadPermission(
					resourceNameForPermissions);
			AssociationData associationData = this.associationReadplatformService
					.retrieveAssociationsDetailsWithSerialNum(clientId,
							serialNumber);
			Collection<EnumValuesData> enumValuesDatas = this.enumReadplaformService
					.getEnumValues(EnumValuesConstants.ENUMVALUE_PROPERTY_DEVICE_SWAP);
			associationData.addEnumValuesDatas(enumValuesDatas);
