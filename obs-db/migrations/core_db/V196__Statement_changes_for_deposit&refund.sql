...
union all select 
    ...
    bdr.id AS transId,
    date_format(bdr.transaction_date,
            '%Y-%m-%d') AS transDate,
    'DEPOSIT&REFUND' AS transType,
    (case bdr.transaction_type
        when 'Deposit' then bdr.debit_amount
        when 'Refund' then -(bdr.credit_amount)
    end) AS amount,
...
where
    ((bdr.transaction_date <= now()) and bdr.description in('Deposit','Refund') and isnull(bdr.bill_id)) 
